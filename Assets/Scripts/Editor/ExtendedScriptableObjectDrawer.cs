	...
	public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
	{
		...
		else
		{
			if (GUI.Button(buttonRect, "Create"))
			{
				...
				{
					string selectedAssetPath = "Assets/ItemsData";
					MonoScript ms = MonoScript.FromMonoBehaviour((MonoBehaviour)property.serializedObject.targetObject);
					selectedAssetPath = System.IO.Path.GetDirectoryName(AssetDatabase.GetAssetPath(ms));
					newObject = CreateAssetWithSavePrompt(type, selectedAssetPath);
				}
				else
				{
					newObject = ScriptableObject.CreateInstance(type);
//					newObject.name = System.Guid.NewGuid().ToString();
				}
				AssetDatabase.AddObjectToAsset(newObject, property.serializedObject.targetObject);
